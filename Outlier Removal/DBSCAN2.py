...
output_dir = "DBSCAN_Results_2" # Changed output directory name
...
def indices_confusion_matrix(y_true, y_pred, method_name, execution_time, original_tags, eps, output_dir):
    """
    ...
    print(f"\n--- Metrics for eps = {eps:.2f} ---")
    print("\nMisclassified Tag Distribution:")
    print(tag_counts)

    # Save metrics to a text file
    metrics_filename = os.path.join(output_dir, f"DBSCAN_eps_{eps:.2f}_metrics_2.txt") # Changed filename to _2
    with open(metrics_filename, "w") as f:
        f.write(f"Metrics for eps = {eps:.2f}\n")
    ...
    plt.title(f"Confusion Matrix: {method_name} (eps={eps:.2f})")

    # Save the confusion matrix plot to a PNG file
    plot_filename = os.path.join(output_dir, f"DBSCAN_eps_{eps:.2f}_confusion_matrix_2.png") # Changed filename to _2
...
pcd = o3d.geometry.PointCloud()
pcd.points = o3d.utility.Vector3dVector(df_merged[['X', 'Y', 'Z']].values)
pcd.colors = o3d.utility.Vector3dVector(df_merged[['Red', 'Green', 'Blue']].values)

# Create true labels (1 for Outlier, 0 for others)
...
for e in range(10):
    eps = 0.2 * (e + 1) # Start from 0.2
    eps_vec.append(eps)
    min_points = 35  # Minimum number of points to form a cluster
    method_name = f'DBSCAN_eps_{eps:.2f}_Method'
    ...
    # Convert DBSCAN labels to outlier predictions (-1 = noise/outlier, others = inlier)
    predicted_labels = np.where(labels == -1, 1, 0)  # Map noise points to outliers (1)
    ...
    print(f"Point cloud has {max_label + 1} clusters (+1 for noise) for eps = {eps:.2f}")
    colors = plt.get_cmap("tab20")(labels / (max_label if max_label > 0 else 1))
    colors[labels < 0] = 0  # Set noise points to black

    # Generate confusion matrix with original tags
    tpr, fpr = indices_confusion_matrix(y_true, predicted_labels, method_name, execution_time, df_merged['Tag'], eps, output_dir)
    tpr_values.append(tpr)
    fpr_values.append(fpr)

# ROC curve plotting
plt.figure(figsize=(8, 6))
plt.plot(fpr_values, tpr_values, marker='o', linestyle='-', color='darkorange', lw=2, label='ROC curve (DBSCAN)')
plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
plt.xlim([0.0, 1.0]) # Ensured xlim is [0, 1]
plt.ylim([0.0, 1.05]) # Ensured ylim starts from 0
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver Operating Characteristic (ROC) Curve - DBSCAN (Varying eps)')
plt.legend(loc="lower right")
plt.savefig(os.path.join(output_dir, "DBSCAN_ROC_curve_2.png"), dpi=300, bbox_inches='tight') # Changed filename to _2
plt.show()

print("\nResults saved in 'DBSCAN_Results_2' folder.") # Changed folder name in print statement