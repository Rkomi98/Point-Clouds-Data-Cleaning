...
def add_noise_to_dataframe(df, noise_percentage=10, position_noise_std=0.5, color_noise_std=0.05):
    ...
    return df_merged
def add_outliers_to_dataframe(df, num_outlier_clusters=4, cluster_size_range=(10, 50), 
    ...
    return df_merged 
def indices_confusion_matrix(y_true, y_pred, method_name, execution_time):
    """
    ...
    metrics_filename = f"{method_name}_metrics.txt"
    ...
    plot_filename = f"{method_name}_confusion_matrix.png"
...
# Example usage:
X, y = create_sampled_dataset(df_merged)
...
indices_confusion_matrix(y, predicted_labels, 'Kernel Based method', execution_time)
