...
def indices_confusion_matrix(y_true, y_pred, method_name, execution_time, original_tags, eps, output_dir):
    ...
    metrics_filename = os.path.join(output_dir, f"DBSCAN_eps_{eps:.2f}_metrics_45.txt")
    ...
    plot_filename = os.path.join(output_dir, f"DBSCAN_eps_{eps:.2f}_confusion_matrix_45.png")
...
for e in range(10):
    ...
    # Create visualizer and take screenshot
    '''
    ...
    vis.poll_events()
    vis.update_renderer()
    vis.capture_screen_image(os.path.join(output_dir, f"dbscan_eps_{eps:.2f}_clusters_45.png"))
    vis.destroy_window()
    '''
...
plt.savefig(os.path.join(output_dir, "DBSCAN_ROC_curve_45.png"), dpi=300, bbox_inches='tight')
plt.show()

print("\nResults saved in 'DBSCAN_Results' folder.")